/**
 * Source partagée pour la génération du PDF "Bon de commande".
 *
 * Le template HTML est inliné ici (et non lu via fs) pour rester compatible
 * avec le packaging Vercel serverless. La version "humaine" se trouve dans
 * `src/templates/bon-de-commande.html` — toute modification ici doit être
 * répliquée là-bas (et inversement).
 *
 * Exposes :
 *   - buildHtml(formData, signatures?) → HTML final prêt à être imprimé
 *   - renderPdfFromHtml(html) → Buffer du PDF (puppeteer-core + chromium)
 */
...
export type SignatureImages = {
  /** Signature vendeur en data URL ou base64 brut. */
  signatureVendeurBase64?: string;
  /** Signature client en data URL ou base64 brut. */
...
/**
 * Construit le tag <img> à insérer en remplacement du placeholder de
 * signature. Si la chaîne est vide, retourne une chaîne vide (le cadre
 * pointillé restera visible côté PDF).
 */
...
export function buildHtml(
  ...
  // Les blocs HTML déjà construits ne doivent PAS passer par le remplacement
  // générique (qui ajoute "—" sur les valeurs vides).
  ...
  // Remplace tout placeholder restant par "—" (sauf signatures déjà traitées)
...
/* ================================================================== */
/*  Render PDF via puppeteer-core + chromium                           */
/* ================================================================== */
